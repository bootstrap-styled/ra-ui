...
export default (url, initialField = 'title') => (driver) => ({
  ...
  getFields() {
    return driver.findElements(this.elements.fields).then((fields) => Promise.all(
      fields.map((field) => field.getAttribute('class').then((classes) => classes
        .split(' ')
        .filter((className) => className.startsWith('ra-field-'))[0]
        .replace('ra-field-', '')))
    ));
  },
  ...
  delete() {
    return driver
      ...
      .then(() => driver.findElement(this.elements.deleteConfirmButton).click())
      .then(() => driver.sleep(200))
      .then(() => driver.wait(until.elementLocated(this.elements.snackbar), 3000))
